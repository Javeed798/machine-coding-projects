...
export default function TwentyFiveFiveClock() {
  const [currenState, setCurrentState] = useState("session");
  ...
  function adjust(e: any) {
    ...
    function adjustTime(currentState: any, currentValue: any) {
    ...
    if (target === "break") {
      setBreakTime((currentValue: any) => adjustTime("break", currentValue));
    } else {
      setSessTime((currentValue: any) => adjustTime("session", currentValue));
    }
  }
      ...
      <div className={styles.clockWrapper}>
        ...
        <div className={styles.timerDisplay}>
          ...
          <p className={lastMinute ? styles.lastMinute : ""}>
            <span>{prependZero(timeLeft[0])}</span>:
            <span>{prependZero(timeLeft[1])}</span>
          </p>
